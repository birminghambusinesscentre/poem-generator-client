...
const amanAssetPaths = [
  join(projectRoot, "public", "background_images", "aman_graduation_bell_courtyard_summer_bg.png"),
  ...
  join(projectRoot, "public", "decorations", "aman_graduation_summer_sun.png"),
];

test("Aman's June 21 graduation party uses a custom multi-asset theme", () => {
  assert.match(themesSource, /id:\s*"aman-graduation-party"/);
  assert.match(amanThemeSource, /name:\s*"Aman's Graduation Party"/);
  assert.match(amanThemeSource, /imagePath:\s*"\/background_images\/aman_graduation_bell_courtyard_summer_bg\.png"/);
  ...
  assert.match(amanThemeSource, /white two-peak event tent/i);
  ...
  assert.match(amanThemeSource, /\/decorations\/aman_graduation_summer_sun\.png/);
